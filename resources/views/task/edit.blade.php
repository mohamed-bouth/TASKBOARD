<x-app-layout>
    <div class="max-w-3xl mx-auto px-4 py-10">
        ...
        <div class="bg-white shadow-lg rounded-lg p-8 border border-gray-200">
            <form action="{{route('task.update' , $task->id )}}" method="post" ><div class="mb-5">
                @csrf
                @method('PUT')
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Title') }}</label>
                    <input name="title" type="text" placeholder="{{ __('Example: Repairing the server...') }}" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition" value="{{ old('title' , $task->title) }}">
                </div>
                @error('title')
                ...
                <div class="mb-5">
                    ...
                    <textarea name="description" rows="4" placeholder="{{ __('Task details...') }}" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition">{{ old('description' , $task->description) }}</textarea>
                </div>
                @error('description')
                    <div class="text-red-500 text-sm">{{ $message }}</div>
                @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input name="expiry_date" type="date" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 outline-none" value="{{ old('expiry_date', $task->expiry_date ) }}">
                    @error('expiry_date')
                        <div class="text-red-500 text-sm">{{ $message }}</div>
                    @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="priority" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 outline-none bg-white">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="status" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 outline-none bg-white">
                            <option value="todo" {{ old('status', $task->status) == 'todo' ? 'selected' : '' }} >{{ __('Todo') }}</option>
